    ...
    <Container fluid className={"pt-4"}>
      <Row>
        <Col xl={9}>
                ...
                <Row className='justify-content-between'>
                  ...
                  {sessionQuery.isSuccess && sessionQuery.data && (
                    <Card.Text>
                      ...
                      set of reports has been picked out for you.
                    </Card.Text>
                  )}
                </Row>
              </Card.Body>
                  ...
                  onClick={() => {
                    if (batchQuery.data && batchQuery.data.results) {
                      setSelectedReadMutation
                        .mutateAsync(objectsToIds(batchQuery.data.results))
                        .then(() => {
                          newBatchMutation.mutate();
                        });
                    }
          ...
          {((!batchMode && reportsQuery.isError) ||
            (batchMode && batchQuery.isError)) && (
            <>
              {batchMode &&
                batchQuery.error &&
                batchQuery.error.response &&
                batchQuery.error.response.status &&
          ...
          {((!batchMode && reportsQuery.isLoading) ||
