      ...
      <Stack direction={"horizontal"} gap={2} className={"justify-content-end"}>
        {props.itemsPerPage > props.total && (
          <small className={"me-2"}>
            {formatNumber(pageNum * props.itemsPerPage + 1)} -{" "}
            {formatNumber(props.total)} of {formatNumber(props.total)}
          </small>
        )}
        {props.total > props.itemsPerPage && (
          <small className={"me-2"}>
            {formatNumber(pageNum * props.itemsPerPage + 1)} -{" "}
            {formatNumber(
              pageNum * props.itemsPerPage + props.itemsPerPage + 1
            )}{" "}
            of {formatNumber(props.total)}
          </small>
        )}
        <Pagination
          className={"mb-0"}
        ...
        >
          ...
          <Pagination.Item disabled aria-disabled='true'>
            {pageNum}
          </Pagination.Item>
          {pageNum + 1 < props.total / props.itemsPerPage && (
            <>
              ...
              <Pagination.Last
                onClick={() =>
                  props.goToPage(
                    Math.ceil(props.total / props.itemsPerPage - 1)
                  )
                }
              />
            </>
      ...
      return (
        ...
        >
          {props.itemsPerPage > props.total && (
            <small className={"me-2"}>
              {formatNumber(
                Number(searchParams.get("page")) * props.itemsPerPage + 1
              )}{" "}
              - {formatNumber(props.total)} of {formatNumber(props.total)}
            </small>
          )}
          {props.total > props.itemsPerPage && (
            <small className={"me-2"}>
              ...
              -{" "}
              {formatNumber(
                Number(searchParams.get("page")) * props.itemsPerPage +
                  props.itemsPerPage +
                  1
              )}{" "}
              of {formatNumber(props.total)}
          ...
          >
            ...
            {Number(searchParams.get("page")) > 0 && (
              <>
                ...
                <Pagination.Prev
                  onClick={() =>
                    props.goToPage(Number(searchParams.get("page")) - 1)
                  }
                />
                <Pagination.Item
                  onClick={() =>
